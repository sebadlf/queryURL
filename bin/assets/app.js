/**
 * emc-service-providers - v2.0.0 - 2015-11-23
 * http://emc.com/
 *
...
angular.module('mConfiguration', []).factory('configuration', [
  '$location',
  '$q',
  function ($location, $q) {
    var serviceInstance = {}, activeURL = '', environment = [
        {
          'environment': 'dev',
...
angular.module('emc_service_providers', [
  ...
  function ($scope, $filter, $q, $timeout, $modal, getResource, $location, CacheSrv, $rootScope, MockSrvApi, configuration) {
      ...
      $scope.data.filtered.main.map(function (provider) {
        ...
        if (matchedFilters && matchedFilters.length) {
          var hasFocusPartner = _.any(matchedFilters, function (matchedFilter) {
              return matchedFilter.focus_partner && matchedFilter.focus_partner.length && matchedFilter.focus_partner[0].toLowerCase() === 'yes';
            });
                ...
                if (!_.isNull($scope.data.filtered.search)) {
                  ...
                  if (_.isEmpty(document.querySelectorAll('main > .app-content.ui-scrollfix'))) {
                    if ($scope.isXSmall()) {
                      ...
                      offset = -jQuery('#headerWrap').height() - jQuery('.app-nav').height() - filtersHeight;
                    }
                  } else {
                    if ($scope.isXSmall()) {
                      if (_.isUndefined($scope.data.selected.xs_nav_open) || _.isNull($scope.data.selected.xs_nav_open)) {
                      ...
                      offset = -jQuery('header').height() - (jQuery('.app-nav').height() - offsetCorrection) - jQuery('.filters-selected').height();
                    }
                  }
    ...
    $scope.confirmAction = function (action) {
      ...
      modal = { result: modalDeferred.promise };
      if (action === 'add') {
